...
class CommentManager extends AbstractEntityManager
{
    ...
    /**
     * Récupère le nombre de commentaire d'un article
     * @param int $id : l'id de l'article sur lequel on veut le nombre de commentaire
    ...
    public function getCommentCount($id) : int
    {
        ...
        $result = $this->db->query($sql, ['id_article' => $id]);
        $commentCount = $result->fetchColumn();
        return $commentCount !== false ? (int) $commentCount : 0;
    }
